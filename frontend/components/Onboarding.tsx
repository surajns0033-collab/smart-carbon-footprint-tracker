...
export const Onboarding: React.FC = () => {
  ...
  const getGreeting = (lang: string) => {
    const greetings: Record<string, string> = {
      ...
      Tamil: 'வரவேற்பு', Telugu: 'స్వాగతం', Kannada: 'ಸ್ವಾಗತ', Malayalam: 'സ്ವಾಗತ',
      ...
      <div className="bg-white rounded-3xl shadow-xl w-full max-w-md overflow-hidden">
        ...
        <form onSubmit={(e) => { e.preventDefault(); if (isStepValid()) handleNext(); }} className="p-8">
          <div className="min-h-[200px]">
            {step === 1 && (
              <div className="space-y-5 animate-fade-in">
                <div>
                  <label className="flex items-center gap-2 text-sm font-bold text-gray-700 mb-2"><User className="text-eco-500" size={18}/> What should we call you?</label>
                  <input 
                    type="text" 
                    placeholder="Enter your name" 
                    className="w-full p-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-eco-500 outline-none text-lg transition-shadow"
                ...
                <div>
                  <label className="flex items-center gap-2 text-sm font-bold text-gray-700 mb-2"><Globe className="text-eco-500" size={18}/> Preferred Language</label>
                  <select 
                    className="w-full p-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-eco-500 outline-none bg-white text-lg transition-shadow cursor-pointer"
            ...
            {step === 2 && (
              <div className="space-y-5 animate-fade-in">
                <div>
                  <label className="flex items-center gap-2 text-sm font-bold text-gray-700 mb-2"><MapPin className="text-eco-500" size={18}/> Where are you from?</label>
                  <select 
                    className="w-full p-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-eco-500 outline-none bg-white text-lg transition-shadow cursor-pointer"
                  ...
                  <p className="text-sm text-gray-500 mt-4 text-center">You can add more details like State, City, and Goals later in Profile Settings.</p>
                </div>
              </div>
            )}
          </div>
          ...
          <button 
            ...
            className="w-full mt-8 bg-eco-600 hover:bg-eco-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold py-4 px-4 rounded-xl flex items-center justify-center gap-2 transition-all transform hover:scale-[1.02] active:scale-[0.98] text-lg shadow-md hover:shadow-lg"
          >
